 ...
 */
program
  .name('modbus-browser read-discrete-inputs')
  .description('Reads the value of the discrete inputs between the given start and end addresses.')
  ...
  .option('-a, --start-address <address>', 'The start address of the discrete inputs to be read.')
  .option('-c, --count <count>', 'The amount of coils to be read.')
  .option('-m, --monitor', 'Causes the `mobus-browser` to continuously monitor the discrete inputs at the given address.')
  .option('-i, --interval <interval>', 'Specifies the interval in milliseconds at which `mobus-browser` is continuously dumping the discrete inputs values when monitoring is enabled.')
  .parse(process.argv);
...
chain.use((input, output) => {
  ...
  intervalFn(() => {
    input.client.readDiscreteInputs(
      ...
      console.log(`\n${hexdump(res.response._body._discrete)}\n`);
      if (!program.monitor) {
        // Closing the socket.
